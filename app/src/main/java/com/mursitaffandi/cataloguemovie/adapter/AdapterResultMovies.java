...
import com.mursitaffandi.cataloguemovie.util.MyApplication;
...
//        Glide.with(mContext).load(BuildConfig.BASE_URL_IMG + Constant.SIZE_IMAGE_POSTER + resultList.get(position).getPosterPath()).into(holder.imageView);
        ...
        MyApplication.getPicasso().load(BuildConfig.BASE_URL_IMG + Constant.SIZE_IMAGE_POSTER + resultList.get(position).getPosterPath()).into(holder.imageView);
